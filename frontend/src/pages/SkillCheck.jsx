...
export default function SkillCheck() {
  ...
  const { activeTrackId, tracks } = useApp();

  // Resolve a role that actually has a quiz bank (safe fallback).
  const quizRole = QUIZ_QUESTIONS[activeTrackId] ? activeTrackId : "ml";
  const questions = QUIZ_QUESTIONS[quizRole];
  const track = tracks[quizRole] || tracks[activeTrackId] || Object.values(tracks)[0] || null;
    ...
    return (
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-8 flex flex-col gap-2 sm:flex-row">
          ...
          <Button size="lg" className="flex-1" onClick={() => navigate("/path-select")}>
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <div className="mb-6 text-center">
        <Badge variant="accent" className="mb-3">
          <Sparkles className="size-3.5" /> Skill check · {track?.label}
        </Badge>
        <h1 className="font-display text-2xl font-bold tracking-tight sm:text-3xl">
